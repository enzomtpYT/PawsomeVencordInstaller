...
func init() {
	...
	if sudoUser != "" {
		if sudoUser == "root" {
			panic("PawsomeVencordInstaller must not be run as the root user. Please rerun as normal user. Use sudo or doas to run as root.")
		}

		fmt.Println("PawsomeVencordInstaller was run with root privileges, actual user is", sudoUser)
		...
		panic("PawsomeVencordInstaller was run as root but neither SUDO_USER nor DOAS_USER are set. Please rerun me as a normal user, with sudo/doas, or manually set SUDO_USER to your username")
	}
	Home = os.Getenv("HOME")
