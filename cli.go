...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install PawsomeVencord")
	var updateFlag = flag.Bool("repair", false, "Repair PawsomeVencord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall PawsomeVencord")
	...
	if *versionFlag {
		fmt.Println("PawsomeVencordInstaller Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2026 Vendicated, thororen1234, enzomtp Vencord, Equicord and PawsomeVencord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update PawsomeVencordInstaller' option to update, or run with --update-self")
			}
		}()

		choices := []string{
			"Install PawsomeVencord",
			"Repair PawsomeVencord",
			"Uninstall PawsomeVencord",
			"Install OpenAsar",
			"Uninstall OpenAsar",
			"View Help Menu",
			"Update PawsomeVencordInstaller",
			"Quit",
		}
		...
		case "Quit":
			return
		case "Update PawsomeVencordInstaller":
			if err := UpdateSelf(); err != nil {
				Log.Error("Failed to update self:", err)
				exitFailure()
			}
	...
	if install {
		...
		Log.Info("Downloading latest PawsomeVencord files...")
